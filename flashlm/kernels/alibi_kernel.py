...
@triton.jit
def alibi_bias_kernel(bias_ptr, slopes_ptr, batch_size, num_heads, seq_len, stride_bias_batch, stride_bias_head, stride_bias_i, stride_bias_j, BLOCK_SIZE_J: tl.constexpr):
    ...
    bias_offset = batch_idx * stride_bias_batch + head_idx * stride_bias_head + i_idx * stride_bias_i
...
class TritonALiBiFunction(torch.autograd.Function):
    ...
    def forward(ctx, slopes, batch_size, num_heads, seq_len):
        bias = torch.empty(batch_size, num_heads, seq_len, seq_len, device=slopes.device, dtype=slopes.dtype)
        BLOCK_SIZE_J = triton.next_power_of_2(seq_len)
        grid = (batch_size, num_heads, seq_len)
        alibi_bias_kernel[grid](bias, slopes, batch_size, num_heads, seq_len, bias.stride(0), bias.stride(1), bias.stride(2), bias.stride(3), BLOCK_SIZE_J=BLOCK_SIZE_J)
        return bias
