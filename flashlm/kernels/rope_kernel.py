...
@triton.jit
def _rope_rotation_kernel(tensor_ptr, cos_ptr, sin_ptr, output_ptr, batch_size, seq_len, num_heads, head_dim, stride_tensor_batch, stride_tensor_seq, stride_tensor_head, stride_tensor_dim, stride_cos_seq, stride_cos_dim, stride_sin_seq, stride_sin_dim, is_backward: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    ...
    tensor_base = batch_idx * stride_tensor_batch + seq_idx * stride_tensor_seq + head_idx * stride_tensor_head
    ...
    x2 = tl.load(tensor_ptr + tensor_base + dim_offsets + half_dim, mask=mask, other=0.0)
...
def _apply_rope_kernel(tensor, cos, sin, is_backward=False):
    ...
    _rope_rotation_kernel[grid](tensor, cos, sin, output, batch_size, seq_len, num_heads, head_dim, tensor.stride(0), tensor.stride(1), tensor.stride(2), tensor.stride(3), cos.stride(0), cos.stride(1), sin.stride(0), sin.stride(1), is_backward=is_backward, BLOCK_SIZE=BLOCK_SIZE)
    return output
