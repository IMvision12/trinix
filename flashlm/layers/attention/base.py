...
class FastBaseAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float=0.0, bias: bool=True, kernel_type: str='flash', causal: bool=False):
        super().__init__()
    ...
    def _apply_flash_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attn_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
    ...
    def _apply_triton_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attn_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
        warnings.warn('Triton kernel not implemented yet, falling back to PyTorch')
        return self._apply_pytorch_attention(q, k, v, attn_mask)

    def _apply_pytorch_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attn_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
        batch_size, seq_len, num_heads, head_dim = q.shape
    ...
    def forward_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attn_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
