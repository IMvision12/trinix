...
class FastGroupedQueryAttention(FastBaseAttention):
    def __init__(
        ...
        if head_dim is not None:
            ...
            super().__init__(
                temp_embed, num_heads, dropout, bias, kernel_type, causal,
                position_method, max_seq_len, rope_base, max_relative_position,
                use_sliding_window, sliding_window_size, qk_layer_norm,
                use_triton_embeddings, False  # GQA doesn't use add_zero_attn
            )
            self.head_dim = head_dim
        else:
            super().__init__(
                embed_dim, num_heads, dropout, bias, kernel_type, causal,
                position_method, max_seq_len, rope_base, max_relative_position,
                use_sliding_window, sliding_window_size, qk_layer_norm,
                use_triton_embeddings, False  # GQA doesn't use add_zero_attn
            )
