...
class FastLayerNorm(nn.Module):
    ...
    def __init__(self, normalized_shape: int, eps: float=1e-05, elementwise_affine: bool=True, triton: bool=False):
        super().__init__()
        ...
        self.triton = triton
        self.pytorch_layernorm = nn.LayerNorm(normalized_shape=normalized_shape, eps=eps, elementwise_affine=elementwise_affine)
    ...
    def _triton_forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        output = TritonLayerNormKernel.apply(input_2d, self.pytorch_layernorm.weight if self.elementwise_affine else None, self.pytorch_layernorm.bias if self.elementwise_affine else None, self.eps)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if input.shape[-len(self.normalized_shape):] != self.normalized_shape:
            raise ValueError(f'Expected input shape ending with {self.normalized_shape}, but got {input.shape}')
