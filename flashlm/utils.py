...
def get_available_kernels() -> Dict[str, bool]:
    kernels = {'flash': False, 'triton': False, 'pytorch': True}
    try:
        import flash_attn
        kernels['flash'] = True
    except ImportError:
...
def benchmark_kernels(embed_dim: int=512, num_heads: int=8, seq_len: int=1024, batch_size: int=4, num_runs: int=100, device: str='cuda') -> Dict[str, float]:
    ...
    for kernel_type in ['pytorch', 'flash', 'triton']:
        if not available_kernels[kernel_type]:
            continue
        attn = FastGroupedQueryAttention(embed_dim=embed_dim, num_heads=num_heads, num_kv_heads=num_heads // 2, kernel_type=kernel_type).to(device)
        with torch.no_grad():
            for _ in range(10):
                _ = attn(x)
