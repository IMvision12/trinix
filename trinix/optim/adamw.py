...
from ..kernels.adamw_kernel import TritonAdamWKernel
...
class FastAdamW(Optimizer):
    def __init__(
        ...
        super(FastAdamW, self).__init__(params, defaults)
        self.use_triton = use_triton

    def _check_triton_availability(self, param: torch.Tensor) -> bool:
        if not self.use_triton:
            return False
        if not TritonAdamWKernel.is_available():
            return False
    ...
    def step(
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError("FastAdamW does not support sparse gradients")
